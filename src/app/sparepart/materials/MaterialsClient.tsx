...
import type { SparepartCategory, SparepartItem, SparepartItemInput } from "@/lib/types";
...
import { sortSparepartItems } from "@/lib/sparepart/sort";
...
export default function MaterialMasterPage() {
  ...
  const load = useCallback(
    async (filters: { q: string; category: string }) => {
      ...
      try {
        ...
        const data = await apiGetAbs<ListResponse>(`/api/sparepart/materials?${params.toString()}`);
        setRows(data.rows);
      } catch (e) {
  ...
  const uploadMaterialImage = async (itemId: number, file: File) => {
    ...
    if (!res.ok) {
      throw new Error((data as { error?: string }).error || t.toast.saveFailed);
    }
  };
  ...
  const handleCreate = async (
    ...
    const created = await apiSendAbs<{ id: number }>("/api/sparepart/materials", "POST", input);
    if (extras.file) {
  ...
  return (
    <SparepartGate allow={(a) => a.canViewSparepartMaterials}>
      <div>
        ...
        {formOpen && (editRow ? canUpdateSparepartMaterial : canCreateSparepartMaterial) ? (
          <ItemForm
            initial={editRow}
            onClose={() => {
              setFormOpen(false);
