...
export default function StorageLocationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiGetAbs<ListResponse>("/api/sparepart/storage-locations?active=0");
      setRows(data.rows);
    } catch (e) {
  ...
  const remove = async (row: SparepartStorageLocation) => {
    ...
    try {
      await apiSendAbs(`/api/sparepart/storage-locations?id=${row.id}`, "DELETE");
  ...
  const sortedRows = useMemo(() => {
    ...
    return [...rows].sort((a, b) => {
      ...
      if (key === "is_active") {
        ...
        cmp = compareStrings(localizedName(a, lang), localizedName(b, lang));
      } else {
          ...
          {open ? (
            ...
            >
              ...
              >
                ...
                <span className="normal-case tracking-normal">{t.common.sortAsc}</span>
              </button>
              <button
                type="button"
                role="menuitem"
              ...
              >
                ...
                <span className="normal-case tracking-normal">{t.common.sortDesc}</span>
              </button>
            </div>
          ) : null}
        </div>
    ...
    <SparepartGate allow={(a) => a.canManageSparepartLocations}>
      <div>
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-lg font-semibold text-text">{t.sparepart.locationsTitle}</h1>
            ...
            <table className="w-full border-collapse">
              <thead className="border-b border-border-subtle bg-bg/40">
                <tr>
                  ...
                  <th className={th}>
                    {renderSortHeader(t.sparepart.locationActive, "is_active")}
                  </th>
              ...
              <tbody>
                {sortedRows.map((row) => (
                  <tr key={row.id} className="border-b border-border-subtle/60 last:border-0">
                    ...
                    <td className={td}>{row.is_active ? t.common.yes : t.common.no}</td>
                    <td className={td}>
                      <button
                        type="button"
