import type { Dict } from "./en";

const cn: Dict = {
  common: {
    appName: "IT 运营指挥中心",
    loading: "加载中...",
    save: "保存",
    cancel: "取消",
    add: "新增",
    edit: "编辑",
    delete: "删除",
    confirm: "确认",
    actions: "操作",
    search: "搜索",
    filter: "筛选",
    reset: "重置",
    apply: "应用",
    all: "全部",
    none: "无",
    backToOverview: "返回总览",
    noData: "所选筛选条件下没有数据",
    error: "出现错误",
    required: "此项为必填",
    rowsPerPage: "每页行数",
    previous: "上一页",
    next: "下一页",
    pageOf: "第 {page} 页，共 {total} 页",
    showingRange: "显示第 {from}–{to} 条，共 {total} 条",
    downloadTemplate: "下载模板",
    import: "导入",
    importing: "导入中...",
    export: "导出",
    exporting: "导出中...",
    sortAsc: "升序排序",
    sortDesc: "降序排序",
    yes: "是",
    no: "否",
    confirmDelete: "将停用此记录。是否继续？",
  },
  nav: {
    overview: "总览",
    itsm: "IT服务管理",
    dailyOperation: "日常运营",
    management: "日常活动",
    analysis: "数据洞察",
    master: "配置管理",
    masterUsers: "用户",
    masterCategories: "类别",
    masterSubcategories: "子类别",
    moduleManagement: "管理",
    moduleAnalysis: "分析",
    masterData: "主数据",
    safety: "安全",
    sparepart: "备件管理",
    sparepartStock: "库存总览",
    sparepartPost: "过账物料凭证",
    sparepartDocuments: "物料凭证",
    sparepartMaterials: "物料主数据",
    sparepartLocations: "库存地点",
    organization: "组织管理",
    report: "报表",
    training: "培训",
    settings: "设置",
    settingsRoles: "角色",
    settingsAccounts: "账号",
    comingSoon: "即将推出",
    soon: "即将推出",
    expandSidebar: "展开侧边栏",
    collapseSidebar: "收起侧边栏",
  },
  sparepart: {
    stockTitle: "库存总览",
    stockDesc: "按物料与库存地点查看当前库存 — 库存仅通过过账变更",
    postTitle: "过账物料凭证",
    postDesc: "过账收货（101）、发货（201）或转储（311）",
    documentsTitle: "物料凭证",
    documentsDesc: "已过账的物料移动历史",
    materialsTitle: "物料主数据",
    materialsDesc: "维护物料编码、名称、品牌、默认地点等主数据",
    locationsTitle: "库存地点",
    locationsDesc: "维护库存地点主数据",
    code: "物料",
    name: "描述",
    brand: "品牌",
    model: "型号",
    location: "库存地点",
    defaultLocation: "默认地点",
    locationCode: "地点代码",
    locationName: "地点名称",
    locationActive: "启用",
    stockByLocation: "按地点库存",
    noBalances: "暂无库存余额。",
    notes: "备注",
    note: "行备注",
    stockIn: "累计收货",
    stockOut: "累计发货",
    stockCurrent: "可用库存",
    qty: "数量",
    date: "过账日期",
    item: "物料",
    totalItems: "物料数",
    zeroStock: "零库存",
    totalUnits: "现存总量",
    lowStockOnly: "仅显示零库存",
    movementType: "移动类型",
    movement101: "101 — 收货",
    movement201: "201 — 发货（已使用/消耗）",
    movement311: "311 — 库存地点转储",
    movement102: "102 — 收货冲销",
    movement202: "202 — 发货冲销",
    movement312: "312 — 转储冲销",
    fromLocation: "发出地点",
    toLocation: "接收地点",
    headerText: "抬头文本",
    recipient: "领用人 / 使用方",
    recipientHint: "发货必填 — 人员、部门或用途",
    addLine: "添加行",
    removeLine: "删除",
    post: "过账",
    posting: "过账中...",
    postSuccess: "已过账凭证 {doc}。",
    viewDocument: "查看凭证",
    reverseDocument: "冲销凭证",
    reversing: "冲销中...",
    reverseSuccess: "冲销凭证已过账为 {doc}。",
    docNumber: "凭证号",
    lines: "行数",
    totalQty: "总数量",
    documentDetail: "物料凭证",
    allTypes: "全部移动类型",
    goPost: "过账物料凭证",
    importTitle: "导入物料主数据",
    importHint:
      "上传含 Kode Barang / 编码 与 Nama Barang / 名称 的 Excel。地点须为单一库存地点（不可逗号分隔）。库存数量将生成收货凭证（101）。",
  },
  settings: {
    rolesTitle: "角色",
    rolesDesc: "定义角色并分配权限",
    accountsTitle: "账号",
    accountsDesc: "为每个登录账号分配角色",
    roleName: "角色",
    roleDescription: "说明",
    permissions: "权限",
    employeeNo: "工号",
    accountName: "姓名",
    active: "启用",
    lastLogin: "上次登录",
    yes: "是",
    no: "否",
    adminOnly: "仅管理员可访问设置。",
    deleteRoleConfirm: "该角色将被永久删除，是否继续？",
  },
  dailyOp: {
    title: "日常运营",
    subtitle: "跟踪、分析和管理日常运营活动",
    manageTitle: "日常活动",
    manageDesc: "记录和管理每日运营活动",
    analysisTitle: "运营洞察",
    analysisDesc: "查看运营趋势、绩效和数据分析",
    masterDesc: "管理用户、类别、子类别及其他配置数据",
  },
  overview: {
    title: "总览",
    subtitle: "总览看板 — 点击卡片展开，然后进入模块详情。",

    itsmOverview: "ITSM 总览",
    dailyOperationOverview: "日常运营总览",

    totalTicket: "工单总数",
    openTicket: "未关闭工单",
    closedTicket: "已关闭工单",
    slaCompliance: "SLA 达成率",

    thisMonthTasks: "本月任务数",
    completed: "已完成",
    totalUsers: "用户总数",
    avgTasks: "平均任务数",

    ticketByDepartment: "本月各部门工单",
    taskByDepartment: "本月各部门任务",

    topPicTicket: "工单数量最多的负责人",
    topPicTask: "任务数量最多的负责人",

    ticketTrend: "最近 7 天工单趋势",
    taskStatus: "本月任务状态",

    open: "未关闭",
    closed: "已关闭",
    pending: "待处理",
    inProgress: "进行中",
    done: "完成",

    clickDetails: "点击任意卡片查看更多详情。",
  },
  fields: {
    pic: "负责人",
    division: "部门",
    category: "类别",
    subcategory: "子类别",
    description: "描述",
    descriptionEn: "描述（英文）",
    descriptionCn: "描述（中文）",
    solution: "解决方案",
    solutionEn: "解决方案（英文）",
    solutionCn: "解决方案（中文）",
    type: "类型",
    status: "状态",
    startTime: "开始时间",
    endTime: "结束时间",
    createdAt: "创建时间",
    nameEn: "名称（英文）",
    nameCn: "名称（中文）",
    dateRange: "日期范围",
    from: "从",
    to: "至",
  },
  analysis: {
    total: "任务总数",
    completed: "已完成",
    inProgress: "总用户数",
    pending: "平均任务数",
    byStatus: "按状态统计",
    byDivision: "按部门统计",
    byType: "按类型统计",
    trend: "任务趋势",
    topPic: "负责人排行",
    avgDuration: "平均时长（分钟）",
    categoryDistribution: "类别分布",
    userRanking: "用户排名",
    subCategoryDistribution: "子类别分布",
    divisionDistribution: "部门分布",
    divisionAnalysis: "部门统计",
    durationAnalysis: "持续时间分析",
    thisWeek: "本周",
    thisMonth: "本月",
    thisYear: "今年",
    customRange: "自定义范围",
    allDepartment: "全部部门",
    mes: "MES",
    it: "IT",
    intelligentLogistics: "智能物流",
    clickToExpand: "点击放大",
    downloadPng: "下载 PNG",
    copyImage: "复制图片",
    copying: "复制中...",
    copied: "已复制",
    copyFailed: "复制失败",
    others: "其他",
    showingTop: "显示前 {n} 项",
  },
  confirmDelete: {
    title: "删除记录？",
    message: "该记录将从列表中移除。是否继续？",
  },
  validation: {
    required: "请填写所有必填项。",
    startBeforeEnd: "开始时间必须早于结束时间。",
    enHasChinese: "英文内容不能包含中文字符。",
    cnNeedsChinese: "中文内容必须包含中文字符。",
    invalidDateTime: "请输入有效的日期和时间。",
    fieldRequired: "此项为必填。",
  },
  toast: {
    createSuccess: "记录创建成功。",
    updateSuccess: "记录更新成功。",
    saveFailed: "保存失败。",
    networkError: "网络异常，请重试。",
    validationFailed: "请修正标红的字段。",
    importSuccess: "成功导入 {count} 条记录。",
    importFailed: "导入失败，未保存任何记录。",
    templateDownloadFailed: "模板下载失败。",
    exportFailed: "导出失败。",
    exportEmpty: "当前筛选条件下没有可导出的记录。",
  },
  importModal: {
    title: "导入活动",
    selectFile: "选择 Excel（.xlsx）文件",
    hint: "请使用“下载模板”生成的文件。所有行必须有效——任意一行失败则整批不保存。",
    upload: "上传并导入",
    rowError: "第 {row} 行",
  },
};

export default cn;
