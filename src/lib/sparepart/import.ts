...
import { DEFAULT_UOM_CODE, normalizeUomCode } from "./uoms";
...
function findNameEnCol(headers: string[]): number {
  ...
  const hasBilingual = headers.some((h) => h.includes("name en") || h.includes("name cn"));
...
function findBrandEnCol(headers: string[]): number {
  ...
  if (exact >= 0) return exact;
  const hasBilingual = headers.some((h) => h.includes("brand en") || h.includes("brand cn"));
  if (hasBilingual) return -1;
...
export async function parseSparepartItemsWorkbook(
  ...
  { ok: true; items: ParsedImportItem[] } | { ok: false; error: string; errors: ImportRowError[] }
> {
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.load((Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer)) as never);

  const sheet =
    workbook.worksheets.find((ws) => /item|stock|stok|备品|清单/i.test(ws.name)) ??
  ...
  for (let r = 2; r <= lastRow; r++) {
    ...
    const rawUom = uomCol >= 0 ? cellText(row.getCell(uomCol + 1).value) : "";
    const uom_code = rawUom ? (normalizeUomCode(rawUom) ?? rawUom.toUpperCase()) : DEFAULT_UOM_CODE;
    ...
    items.push({
      ...
      name_en,
      name_cn,
      brand_en: brandEnCol >= 0 ? cellText(row.getCell(brandEnCol + 1).value) : "",
      brand_cn: brandCnCol >= 0 ? cellText(row.getCell(brandCnCol + 1).value) : "",
