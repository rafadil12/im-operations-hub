...
export function parseGoodsMovementBody(
  ...
  if (!postingDate) {
    throw new SparepartPostingError("Posting date must be YYYY-MM-DD or YYYY-MM-DD HH:mm:ss.");
  }
  ...
  const clientRequestId = body.client_request_id
    ...
    : undefined;
  const reversalOfDocId = body.reversal_of_doc_id ? Number(body.reversal_of_doc_id) : undefined;

  if (movementType === "201" && !recipient) {
    throw new SparepartPostingError("Recipient / Used by is required for Goods Issue (201).");
  }

  if (isReversalType(movementType)) {
    if (!reversalOfDocId || !Number.isInteger(reversalOfDocId) || reversalOfDocId <= 0) {
      throw new SparepartPostingError("reversal_of_doc_id is required for reversal movements.");
    }
  }
  ...
  if (Array.isArray(body.lines) && body.lines.length > 10) {
    throw new SparepartPostingError("A document can have at most 10 line items.");
  }

  const lines: SparepartGoodsMovementLineInput[] = (body.lines ?? []).map((line, index) => {
    ...
    if (!Number.isInteger(qty) || qty <= 0) {
      throw new SparepartPostingError(`Line ${index + 1}: quantity must be a positive integer.`);
    }
  ...
  return {
    ...
    recipient: movementType === "201" || movementType === "202" ? recipient : "",
    lines,
...
async function nextDocNumber(conn: PoolConnection, postingDate: string): Promise<string> {
...
async function loadActiveLocation(
  ...
  const loc = rows[0] as
    ...
    | undefined;
  if (!loc) {
    throw new SparepartPostingError(`Line ${lineNo}: storage location not found.`, 404);
  }
  if (!loc.is_active) {
    throw new SparepartPostingError(`Line ${lineNo}: storage location ${loc.code} is inactive.`);
  }
  return {
    id: loc.id,
    code: loc.code,
...
function locationLabel(loc: { code: string; name_en: string; name_cn?: string }): string {
...
async function syncItemStockCurrent(conn: PoolConnection, itemId: number): Promise<void> {
  await conn.query(
...
async function buildReversalFromDoc(
  ...
  const doc = docs[0] as
    { id: number; movement_type: MovementType; recipient: string | null } | undefined;
  if (!doc) {
...
export async function reverseMaterialDocument(
  ...
  const doc = docs[0] as
    { id: number; doc_number: string; movement_type: MovementType } | undefined;
  if (!doc) {
  ...
  if (doc.movement_type === "101") reversalType = "102";
    ...
    throw new SparepartPostingError("Only forward documents (101/201/311) can be reversed.");
  }
