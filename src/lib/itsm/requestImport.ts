...
import { isServiceRequestValue } from "./serviceRequest";
...
async function workbookToMatrix(buffer: ArrayBuffer | Buffer): Promise<unknown[][] | null> {
...
export async function parseItsmRequestWorkbook(
  ...
  for (let r = found.headerRowIndex + 1; r < matrix.length; r++) {
    ...
    rows.push({
      ...
      is_service_request: parseServiceRequest(getCell(raw, found.colMap.is_service_request)),
    });
  }
