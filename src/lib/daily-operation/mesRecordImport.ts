...
export type ImportParseResult =
  { ok: true; rows: MesDataInput[] } | { ok: false; errors: ImportRowError[] };
...
function findByName<T extends Named>(items: T[], name: string): T | undefined {
  ...
  return items.find(
    (item) =>
      item.name_en?.trim().toLowerCase() === key || item.name_cn?.trim().toLowerCase() === key
  );
}
...
export async function buildActivitiesTemplate(masters: Masters): Promise<Buffer> {
  ...
  const divisionById = new Map(masters.divisions.map((d) => [d.id, displayName(d)]));
  const categoryById = new Map(masters.categories.map((c) => [c.id, displayName(c)]));
  ...
  for (let i = 0; i < maxLen; i++) {
    ...
    note.addRow([
      ...
      user
        ? `${displayName(user)}${
            user.division_id ? ` (${divisionById.get(user.division_id) ?? user.division_id})` : ""
          }`
        : "",
...
export async function parseActivitiesWorkbook(
  ...
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.load((Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer)) as never);

  const sheet = workbook.getWorksheet("Activities") ?? workbook.worksheets[0] ?? null;
  if (!sheet) {
    return {
      ok: false,
  ...
  for (let r = 2; r <= rowCount; r++) {
    ...
    const maxIdx = Math.max(...ACTIVITY_HEADERS.map((h) => mapped.map[h] ?? 0), 0);
    ...
    const divisionName = cellToString(getCell(values, mapped.map, "Division"));
    const categoryName = cellToString(getCell(values, mapped.map, "Category"));
    const subcategoryName = cellToString(getCell(values, mapped.map, "Subcategory"));
    const descriptionCn = cellToString(getCell(values, mapped.map, "Description CN"));
    const descriptionEn = cellToString(getCell(values, mapped.map, "Description EN"));
    const solutionCn = cellToString(getCell(values, mapped.map, "Solution CN"));
    const solutionEn = cellToString(getCell(values, mapped.map, "Solution EN"));
    const typeName = cellToString(getCell(values, mapped.map, "Type"));
    const statusName = cellToString(getCell(values, mapped.map, "Status"));
    const startTime = normalizeDateTimeCell(getCell(values, mapped.map, "Start Time"));
    const endTime = normalizeDateTimeCell(getCell(values, mapped.map, "End Time"));
