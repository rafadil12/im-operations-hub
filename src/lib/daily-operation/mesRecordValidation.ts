...
export type MesValidationErrorKey =
  "required" | "startBeforeEnd" | "enHasChinese" | "cnNeedsChinese" | "invalidDateTime";
...
function pushRequired(errors: MesFieldError[], field: MesFieldKey, ok: boolean) {
  if (!ok) errors.push({ field, key: "required" });
}

export function validateMesRecord(values: MesFormValues): MesValidationResult {
  ...
  const startMysql = values.start_time.trim() ? toMysqlDateTime(values.start_time) : null;
  const endMysql = values.end_time.trim() ? toMysqlDateTime(values.end_time) : null;
...
export function firstErrorField(errors: MesFieldError[]): MesFieldKey | null {
  if (!errors.length) return null;
  return FIELD_ORDER.find((f) => errors.some((e) => e.field === f)) ?? errors[0].field;
}
