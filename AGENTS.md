...
- Materials import/template is master data only (Code, Name EN/CN, Brand EN/CN, Model, Category, Min Stock, Notes) — no opening stock or location. Stock changes go through goods movements (101/201/311). Category is a closed set (IT, AGV, ASSEMBLY, MES); omit → IT. Min stock defaults to 0 (low stock = min > 0 and on-hand ≤ min).
