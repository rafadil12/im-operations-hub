...
- Materials import/template is master data only (Code, Name EN/CN, Brand EN/CN, Model, Category, Min Stock, Notes) — no opening stock or location. Stock changes go through goods movements (101/201/311). Import requires Code, Name EN, Name CN, Category, and Min Stock on every row; Brand/Model/Notes may be empty. Category must be IT, AGV, ASSEMBLY, or MES. Active material codes already in the database are rejected; soft-deleted codes may be restored via import. Min stock must be an integer ≥ 0 (low stock = min > 0 and on-hand ≤ min).
