...
const INITIAL_WEEKLY_RECORDS: WeeklyRecord[] = [
    week: 1,
    startDate: "01 Aug",
    endDate: "07 Aug",
...
export default function SafetyManagementPage() {
  const { t } = useLang();
  const [records, setRecords] = useState<WeeklyRecord[]>(INITIAL_WEEKLY_RECORDS);
  ...
  const loadWeeklyData = useCallback(async () => {
    try {
      ...
      setRecords((current) =>
        current.map((record) => {
          const updated: WeeklyRecord = { ...record };
          const weekRows = rows.filter(
            (row) => Number(row.week) === record.week,
          );
          ...
          for (const row of weekRows) {
            ...
            updated[activity.recordKey] = row.status;

            if (activity.dataKey) {
              updated[activity.dataKey] = {
                ...
                pic: row.pic ?? "—",
                fileNames: row.file_name ? [row.file_name] : [],
                fileUrls: row.file_url ? [row.file_url] : [],
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <select defaultValue="August 2026" className={selectClass}><option>August 2026</option><option>July 2026</option><option>June 2026</option></select>
      </header>
      ...
      <section className="relative overflow-hidden rounded-2xl border border-border bg-surface">
        ...
        <div className="relative grid lg:grid-cols-[1fr_330px]">
          <div className="p-6 md:p-7">
            <span className="rounded-md bg-accent/10 px-2.5 py-1 text-[9px] font-semibold uppercase tracking-wide text-accent">August 2026</span>
